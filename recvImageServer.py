...
def saveImageToDownloads(DOWNLOADS_PATH, image):
    imagename = getImageTimestamp() +".png"
    filepath = DOWNLOADS_PATH + "/" + imagename
    cv2.imwrite(filepath, image)
    # print("Saved ", imagename, " to downloads")
    return imagename
...
def startRecvImageServer(DOWNLOADS_PATH, results):
    ...
    # print(f'Starting up the Image receiving server on {server_address[0]}:{server_address[1]}')
    ...
    while Flag:
        ...
        print('Waiting for a connection to Image receiving server...')
        connection, client_address = sock.accept()
        try:
            print(f'Receiving the image from  {client_address[0]}:{client_address[1]}')
            ...
            image_name = saveImageToDownloads(DOWNLOADS_PATH, decoded_image)
            results[0] = decoded_image.shape
            results[1] = image_name
            results[2] = "extra field"
            Flag = False

        finally:
            # clean up the connection
            connection.close()
            print("Successfully received " + image_name +" and saved to downloads-------------------> ^_^")

if __name__=="__main__":
    startRecvImageServer()